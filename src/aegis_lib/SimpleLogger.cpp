...
BOOST_LOG_GLOBAL_LOGGER_INIT(logger, src::severity_logger_mt)
{
  ...
  logger.add_attribute("LineID",
                       attrs::counter<unsigned int>(1));   // lines are sequentially numbered
  ...
  fileSink->locked_backend()->add_stream(boost::make_shared<std::ofstream>(LOGFILE));
  ...
  logging::formatter formatter =
      expr::stream << std::setw(7) << std::setfill('0') << line_id << std::setfill(' ') << " | "
  ...
  consoleSink->set_filter(severity >= SEVERITY_THRESHOLD);
